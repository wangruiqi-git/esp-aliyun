...
//当前时间在一个星期的偏移 second              星期一1点，偏移3600s
static uint32_t utc = 0;
//ntp更新时间点 msecond       aos_now_ms
static uint32_t uptime_on_get_utc= 0;
//aliyun 发来的时间 second
static uint32_t  ntp_time_s=0;
static int timezone = DEFAULT_TIMEZONEOFFSET;

#ifdef CONFIG_ENABLE_COUNTDOWN
...
#ifdef CONFIG_ENABLE_COUNTDOWN_LIST
...
#endif
#ifdef CONFIG_ENABLE_LOCALTIMER
...
#ifdef CONFIG_ENABLE_RANDOM_TIMER
...
void local_timer_parse(const char *timing_string, int save)
{
    ...
    for (j = 0; j < local_timer_arrySize; j++) {
		...
		time_t now = 0;
		time(&now);
        utc_week_second_offset = (utc + (now - uptime_on_get_utc)) % (86400 * 7);
    ...
    if (save == 1){
		...
		int ret = HAL_SetLocalTimerConf(timing_string);
...
int string2time(const char *string)
{
    ...
    if (string[0] > '9' || string[1] > '9' || string[3] > '9' || string[4] > '9')
        return -1;
    strncpy((char *)&hour[0], (char *)&string[0], 2);
    strncpy((char *)&min[0], (char *)&string[3], 2);
...
static void timer_service_event_check()
{
	...
	time_t now = 0;
	time(&now);
    uint32_t ticket = (now - uptime_on_get_utc);
    ...
    if (delay_sec > 1 && delay_sec < 5*SECONDS_OF_MINUTE){
        HAL_Timer_Stop(check_timer);
        HAL_Timer_Start(check_timer, delay_sec * 1000);
    } else if (delay_sec >= 0 && delay_sec <= 1) {
        //aos_msleep(200);
		HAL_SleepMs(200);
